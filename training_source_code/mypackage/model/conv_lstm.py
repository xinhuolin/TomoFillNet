...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, kernel_size, step=1, effective_step=[1], cuda=True):
        ...
        for i in range(self.num_layers):
            name = 'cell{}'.format(i)
            cell = ConvLSTMCell(self.input_channels[i], self.hidden_channels[i], self.kernel_size, self.use_cuda)
...
if __name__ == '__main__':
    # gradient check
    convlstm = ConvLSTM(input_channels=32, hidden_channels=[32, 16, 2], kernel_size=3, step=5,effective_step=[4],cuda = True).cuda()
    loss_fn = torch.nn.MSELoss()

    input = Variable(torch.randn(4, 32, 256, 256)).cuda()
    target = Variable(torch.randn(4, 2, 256, 256)).double().cuda()
